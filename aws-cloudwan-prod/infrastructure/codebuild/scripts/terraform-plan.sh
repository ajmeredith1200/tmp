...
echo "Running Terraform plan for layer ${TF_LAYER} in env ${TF_ENVIRONMENT}"
...
terraform -chdir="infrastructure/${TF_LAYER}" plan \
  -out="${TF_LAYER}-${TF_ENVIRONMENT}-tfplan" \
  -input=false \
  -var-file="./config/${TF_ENVIRONMENT}/terraform.tfvars" \
  -var="layer=${TF_LAYER}" \
