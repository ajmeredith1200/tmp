...
Each area of Cloud WAN is separated into its own layer, with a dedicated deployment pipeline. This allows Terraform deployments to be managed independently through AWS CodePipeline.

A GitLab Runner is used to package the repository whenever changes are pushed to the `main` branch. The packaged codebase is then uploaded to an S3 bucket for consumption by the deployment pipelines.

The pipelines retrieve the packaged source from S3, extract the contents, and execute the Terraform deployment process.

To deploy a layer:

1. Navigate to **CodePipeline** in the AWS Console.
2. Locate the pipeline for the layer you wish to deploy.
3. Select either the **Build** or **Destroy** pipeline as appropriate.
4. Click **Release change**.

This will start the pipeline and run the deployment up to the Terraform **Plan** stage.

Once the plan has completed:

1. Review the generated Terraform plan.
2. Open the **Manual Approval** stage.
3. Select **Approve**.

The pipeline will then continue to the Terraform **Apply** stage and deploy the approved infrastructure changes.

NOTE: To run the destroy workflow on persistence remove the lifecycle rules and push before running this pipeline or it will fail. 
...
CodePipeline
   │
   ├── Source
   ├── Terraform Init (CodeBuild)
   ├── Terraform Validate (CodeBuild)
