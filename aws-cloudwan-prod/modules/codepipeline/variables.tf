variable "deploy_layer" {
  ...
  type        = string
  validation {
    condition     = contains(["platform", "connectivity", "security", "validation", "codepipeline"], var.deploy_layer)
    error_message = "deploy_layer must be one of: platform, connectivity, security, validation, codepipeline."
  }
}
...
variable "deploy_environment" {
  ...
  validation {
    condition     = contains(["preprod", "prd", "prod"], var.deploy_environment)
    error_message = "deploy_environment must be one of: preprod, prd or prod."
  }
}
...
variable "enable_s3_bucket_polling" {
  description = "When true, enable S3 bucket polling for changes"
  type        = bool
  default     = false
}
...
variable "create_gitlab_s3_upload_user" {
  description = "When true, create the shared GitLab S3 upload IAM user/group for static AWS credentials. Only one layer should set this to true."
  type        = bool
  default     = false
}
