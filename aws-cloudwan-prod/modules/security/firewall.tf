...
resource "aws_security_group" "public_allow" {
  name        = var.fw_external_security_group_name
  description = "FW external inbound access (mgmt)"
  ...
  ingress {
    ...
    to_port     = 22
    protocol    = "6"
    cidr_blocks = ["83.98.0.156/32", "86.22.91.19/32", "85.118.20.102/32"]
  }
...
resource "aws_security_group" "internal" {
  name        = var.fw_internal_security_group_name
  description = "FW internal access"
  vpc_id      = aws_vpc.vpc.id
